...
public final class Controller implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        aboutBtn.setOnMouseClicked(e -> {
            ...
            alert.setHeaderText("ViSoAl");
            alert.setContentText(
                    "Visualize Bubble Sort Algorithm, Insertion Sort Algorithm, Selection Sort Algorithm, Shell Sort, Quick Sort Algorithm & Merge Sort Algorithm and see how these work.\n\nTip: If size of array is larger make sure that you decrease the delay time!");
            alert.show();
        });

        randomizeBtn.setTooltip(new Tooltip("Reset the array and Randomize with random values"));
        bubbleSortBtn
                .setTooltip(new Tooltip("\tWorst case\t\t\tBest Case\nTime Complexity: O(n²)\tTime Complexity: O(n)"));
        insertionSortBtn
                .setTooltip(new Tooltip("\tWorst case\t\t\tBest Case\nTime Complexity: O(n²)\tTime Complexity: O(n)"));
        selectionSortBtn
                .setTooltip(new Tooltip("\tWorst case\t\t\tBest Case\nTime Complexity: O(n²)\tTime Complexity: O(n²)"));
        shellSortBtn.setTooltip(
                new Tooltip("\tWorst case\t\t\tBest Case\nTime Complexity: O(n²)\tTime Complexity: O(n log n)"));
        quickSortBtn.setTooltip(
                new Tooltip("\tWorst case\t\t\tBest Case\nTime Complexity: O(n²)\tTime Complexity: O(n log n)"));
        mergeSortBtn.setTooltip(
    ...
    private void changeStyleEffect(int index1, String style1, String borderColor1, int index2, String style2,
            String borderColor2) {
        Platform.runLater(() -> {
            try {
        ...
        new Thread(() -> {
            ...
            for (int i = 0; i < TOTAL_NUMBER_OF_BARS - 1; i++) {
                ...
                for (int j = 0; j < TOTAL_NUMBER_OF_BARS - i - 1; j++) {
                    changeStyleEffect(j, SELECTED_BARS_COLOR_FILL, SELECTED_BARS_BORDER_COLOR, j + 1,
                            SELECTED_BARS_COLOR_FILL, SELECTED_BARS_BORDER_COLOR);
                    delay();
                    if ((int) ((XYChart.Data) series.getData().get(j))
                            .getYValue() > (int) ((XYChart.Data) series.getData().get(j + 1)).getYValue()) {
    ...
    private void insertionSort() {
        ...
        new Thread(() -> {
            ...
            while (i < TOTAL_NUMBER_OF_BARS) {
                ...
                j = i - 1;
                changeStyleEffect(i, SELECTED_BARS_COLOR_FILL, SELECTED_BARS_BORDER_COLOR, j, SELECTED_BARS_COLOR_FILL,
                        SELECTED_BARS_BORDER_COLOR);
                delay();
                while (j >= 0 && (int) ((XYChart.Data) series.getData().get(j)).getYValue() > x) {
                    CountDownLatch latch = new CountDownLatch(1);
                    int finalJ = j;
                    changeStyleEffect(finalJ, SELECTED_BARS_COLOR_FILL, SELECTED_BARS_BORDER_COLOR, finalJ + 1,
                            SELECTED_BARS_COLOR_FILL, SELECTED_BARS_BORDER_COLOR);
                    Platform.runLater(() -> {
                        ParallelTransition pt = swapAnimation(finalJ, finalJ + 1);
                        pt.setOnFinished(e -> latch.countDown());
        ...
        new Thread(() -> {
            ...
            while (i < TOTAL_NUMBER_OF_BARS - 1) {
                ...
                while (j < TOTAL_NUMBER_OF_BARS) {
                    ...
                    changeStyleEffect(finalJ, SELECTED_BARS_COLOR_FILL, SELECTED_BARS_BORDER_COLOR, finalK,
                            SELECTED_BARS_COLOR_FILL, SELECTED_BARS_BORDER_COLOR);
                    delay();
                    if ((int) ((XYChart.Data) series.getData().get(j))
                            .getYValue() < (int) ((XYChart.Data) series.getData().get(k)).getYValue()) {
                        k = j;
                    }
        ...
        new Thread(() -> {
            ...
            while (gap >= 1) {
                ...
                while (i < TOTAL_NUMBER_OF_BARS) {
                    ...
                    changeStyleEffect(i, SELECTED_BARS_COLOR_FILL, SELECTED_BARS_BORDER_COLOR, j,
                            SELECTED_BARS_COLOR_FILL, SELECTED_BARS_BORDER_COLOR);
                    delay();
                    while (j >= 0 && (int) ((XYChart.Data) series.getData().get(j)).getYValue() > x) {
                        CountDownLatch latch = new CountDownLatch(1);
                        int finalJ = j, finalGap = gap;
                        changeStyleEffect(finalJ, SELECTED_BARS_COLOR_FILL, SELECTED_BARS_BORDER_COLOR,
                                finalJ + finalGap, SELECTED_BARS_COLOR_FILL, SELECTED_BARS_BORDER_COLOR);
                        Platform.runLater(() -> {
    ...
    private int partition(int left, int right) {
        ...
        while (left <= right) {
            ...
            changeStyleEffect(left, SELECTED_BARS_COLOR_FILL, SELECTED_BARS_BORDER_COLOR, right,
                    SELECTED_BARS_COLOR_FILL, SELECTED_BARS_BORDER_COLOR);
            delay();
            if (left <= right) {
    ...
    private void mergeOperation(int low, int mid, int high) {
        ...
        for (int x = 0; x < TOTAL_NUMBER_OF_BARS; x++) {
            copySeries.getData().add(
                    new XYChart.Data(Integer.toString(x), (int) ((XYChart.Data) series.getData().get(i)).getYValue()));
        }
        ...
        while (i <= mid && j <= high) {
            changeStyleEffect(i, SELECTED_BARS_COLOR_FILL, SELECTED_BARS_BORDER_COLOR, j, SELECTED_BARS_COLOR_FILL,
                    SELECTED_BARS_BORDER_COLOR);
            delay();
            if ((int) ((XYChart.Data) series.getData().get(i))
                    .getYValue() < (int) ((XYChart.Data) series.getData().get(j)).getYValue()) {
                ((XYChart.Data) copySeries.getData().get(k))
                        .setYValue(((XYChart.Data) series.getData().get(i)).getYValue());
                i++;
                k++;
            } else {
                ((XYChart.Data) copySeries.getData().get(k))
                        .setYValue(((XYChart.Data) series.getData().get(j)).getYValue());
                j++;
                k++;
            }
            changeStyleEffect(i, MAIN_THEME, j, MAIN_THEME);
        ...
        for (int x = low; x <= high; x++) {
            ...
            ((XYChart.Data) series.getData().get(x))
                    .setYValue(((XYChart.Data) copySeries.getData().get(x)).getYValue());
            delay();
        }
    }
    ...
    private ParallelTransition swapAnimation(int d1, int d2) {
        ...
        double a1 = ((XYChart.Data) series.getData().get(d1)).getNode().getParent()
                .localToParent(((XYChart.Data) series.getData().get(d1)).getNode().getBoundsInParent()).getMinX();
        double a2 = ((XYChart.Data) series.getData().get(d1)).getNode().getParent()
                .localToParent(((XYChart.Data) series.getData().get(d2)).getNode().getBoundsInParent()).getMinX();
        ...
        TranslateTransition t1 = new TranslateTransition(Duration.millis(DELAY_TIME),
                ((XYChart.Data) series.getData().get(d1)).getNode());
        t1.setByX(a2 - a1);
        TranslateTransition t2 = new TranslateTransition(Duration.millis(DELAY_TIME),
                ((XYChart.Data) series.getData().get(d2)).getNode());
        t2.setByX(a1 - a2);
        ...
        pt.statusProperty().addListener((e, old, curr) -> {
            if (old == Animation.Status.RUNNING) {
                ...
                ((XYChart.Data) series.getData().get(d2))
                        .setYValue(((XYChart.Data) series.getData().get(d1)).getYValue());
